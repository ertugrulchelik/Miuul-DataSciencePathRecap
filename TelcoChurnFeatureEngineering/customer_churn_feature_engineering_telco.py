...
df = pd.read_csv("/Users/ertugrulcelik/Desktop/Desktop_DataSciencePathRecap/TelcoChurnFeatureEngineering/Telco-Customer-Churn.csv")
df.head()
df.shape
...
# TotalCharges değişkenini float'a dönüştürmek için önce boş olan değerleri tespit ettik
def replace_with_NaN(dataframe, col_name):
    dataframe[col_name] = dataframe[col_name].replace(' ', np.nan)


for col in df.columns:
    replace_with_NaN(df, col)

# Boş değerleri tespit ettikten sonra tip dönüşümü yapılır
df['TotalCharges'] = df['TotalCharges'].astype(float)

# Tip dönüşümü işleminin daha kısa bir yolu
...
#############################################
# Adım 1: Eksik gözlem analizi yapınız.
##############################################
df.isnull().sum()
...
# farkli sekilde eksik degerin doldurulmasi
#df["TotalCharges"].fillna(df["TotalCharges"].median(), inplace=True)  # aylık ödenecek miktarlarıyla totalcharge doldurulailir (daha iyi olur denensin)  veya 11 değişken drop edilebilir
